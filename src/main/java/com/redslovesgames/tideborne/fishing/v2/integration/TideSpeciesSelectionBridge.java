...
import com.redslovesgames.tideboundcompatibility.fishing.BobberGearModifiers;
...
public final class TideSpeciesSelectionBridge {
    ...
    public CatchResult select(com.li64.tide.data.fishing.FishingContext tideContext){
        ...
        FishingGearModifiers leviathan=TideborneFishingGearModifiers.leviathanBait(LeviathanBaitFishing.isEnabledFor(hook,TideboundConfig.get()));
        FishingGearModifiers bobber=BobberGearModifiers.forHook(hook);
        FishingGearModifiers specimenGear=FishingGearModifiers.compose(leviathan,bobber);
        FishingContext context=contextAdapter.context(tideContext).withGearModifiers(specimenGear);FishingEnvironment environment=contextAdapter.environment(tideContext);List<SpeciesProfile> profiles=new ArrayList<>();Map<String,FishData> dataBySpecies=new HashMap<>();
