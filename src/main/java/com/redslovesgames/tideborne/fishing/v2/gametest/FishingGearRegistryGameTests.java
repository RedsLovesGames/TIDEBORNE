...
public final class FishingGearRegistryGameTests implements FabricGameTest {
    ...
    public void sevenRealLoadoutsConvergeAcrossCastFightAndSatchel(TestContext helper) {
        var config = com.redslovesgames.tideboundcompatibility.config.TideboundConfig.get();
        boolean myths = com.redslovesgames.tideboundcompatibility.TideboundCompatibility.isMythsIntegrationActive();
        boolean apex = com.redslovesgames.tideboundcompatibility.TideboundCompatibility.isApexIntegrationActive();
        ...
        for (var build : GearArchetypeCases.builds()) {
            ...
            ItemStack leader = new ItemStack(com.redslovesgames.tideboundcompatibility.fishing.LeaderAttachment.item(build.leader()));
            ...
            for (int i = 0; i < 6; i++) helper.assertTrue(FishingGearRegistry.accepts(
                    com.redslovesgames.tidetraits.satchel.SatchelPreset.SLOTS.get(i), items.get(i)), "Unregistered/invalid gear: " + build.name() + "/" + i);
            ...
            com.redslovesgames.tideboundcompatibility.fishing.LeaderAttachment.set(rod, build.leader());
            ...
            var fight = com.redslovesgames.tideboundcompatibility.fishing.TideborneFishingGearModifiers.forCanonicalFight(hook, config);
            var selection = com.redslovesgames.tideboundcompatibility.fishing.TideborneFishingGearModifiers.forSelection(hook.getContext(), config);
            var preview = com.redslovesgames.tideboundcompatibility.fishing.TideborneFishingGearModifiers.forLoadoutPreview(rod, config);
            ...
            var context = new com.redslovesgames.tideborne.fishing.v2.integration.TideFishingContextAdapter().context(hook.getContext());
            ...
            if (!build.name().equals("Leviathan") || myths) helper.assertTrue(
                    com.redslovesgames.tidetraits.satchel.SatchelGearSummary.archetypes(preview).contains(build.name()), "Missing canonical specialization: " + build.name());
        }
        player.discard();
        helper.complete();
    }
    ...
    public void leaderPersistenceAndLeviathanRuntimeBoundaries(TestContext helper) {
        ...
        rod.set(com.redslovesgames.tidetraits.component.TideTraitsComponents.STEEL_LEADER_ATTACHED, true);
        var legacy = ItemStack.fromNbt(registries, rod.encode(registries)).orElseThrow();
        helper.assertTrue(com.redslovesgames.tideboundcompatibility.fishing.LeaderAttachment.tier(legacy)
                == com.redslovesgames.tideboundcompatibility.fishing.LeaderTier.IRON, "Legacy steel attachment lost during load");
        for (var tier : com.redslovesgames.tideboundcompatibility.fishing.LeaderTier.values()) {
            com.redslovesgames.tideboundcompatibility.fishing.LeaderAttachment.set(rod, tier);
            var restored = ItemStack.fromNbt(registries, rod.encode(registries)).orElseThrow();
            helper.assertTrue(com.redslovesgames.tideboundcompatibility.fishing.LeaderAttachment.tier(restored) == tier,
                    "Leader tier persistence failed: " + tier);
            helper.assertTrue(Boolean.TRUE.equals(restored.get(com.redslovesgames.tidetraits.component.TideTraitsComponents.STEEL_LEADER_ATTACHED)),
        ...
        com.redslovesgames.tideboundcompatibility.fishing.LeaderAttachment.clear(rod);
        helper.assertTrue(!com.redslovesgames.tideboundcompatibility.fishing.LeaderAttachment.has(rod), "Leader removal retained attachment");
        ...
        var config = com.redslovesgames.tideboundcompatibility.config.TideboundConfig.get();
        if (!com.redslovesgames.tideboundcompatibility.TideboundCompatibility.isMythsIntegrationActive()) {
            helper.assertTrue(!com.redslovesgames.tideboundcompatibility.fishing.LeviathanBaitFishing.isEnabledFor(hook, config),
                    "Absent Myths integration activated Leviathan");
            var gear = com.redslovesgames.tideboundcompatibility.fishing.TideborneFishingGearModifiers.forSelection(hook.getContext(), config);
            helper.assertTrue(gear.fishingLuck() == 0 && gear.traitLuck() == 0, "Inactive Leviathan leaked bonuses");
        }
        var settings = com.redslovesgames.tideboundcompatibility.network.TideboundSettingsPayload.fromValues(config, true, true).tag();
    ...
    public void allBobbersAreAppliedOnceAndCapturedFromServerConfig(TestContext helper) {
        ...
        for (var id : FishingGearRegistry.supportedBobberIds()) {
            ...
            var captured = com.redslovesgames.tideboundcompatibility.fishing.BobberGearModifiers.forHook(hook);
            ...
            helper.assertTrue(hook.getLureSpeed() == 3 + expected.namedAdditiveModifier(com.redslovesgames.tideborne.fishing.v2.FishingGearEffects.LURE_BONUS), "Native bobber lure duplicated: " + id);
            var nativeContext = new com.redslovesgames.tideborne.fishing.v2.integration.TideFishingContextAdapter().context(hook.getContext());
            var gear = com.redslovesgames.tideboundcompatibility.fishing.TideborneFishingGearModifiers.forSelection(hook.getContext(), config);
            ...
            helper.assertTrue(com.redslovesgames.tideboundcompatibility.fishing.BobberGearModifiers.forHook(hook).equals(captured), "Cast snapshot changed after equipment changed");
        }
        helper.complete();
    }
    ...
    public void nativeHooksBaitsAndTargetMetadataRemainAuthoritative(TestContext helper) {
        ...
        var adapter = new com.redslovesgames.tideborne.fishing.v2.integration.TideSpeciesProfileAdapter();
    ...
    public void frozenRodAndLineRuntimeComposition(TestContext helper) {
        ...
        var gear = com.redslovesgames.tideboundcompatibility.fishing.TideborneFishingGearModifiers.forCanonicalFight(hook, config);
        ...
        if (!com.redslovesgames.tideboundcompatibility.TideboundCompatibility.isMythsIntegrationActive()) {
            ...
            var optional = com.redslovesgames.tideboundcompatibility.fishing.TideborneFishingGearModifiers.forCanonicalFight(optionalHook, config);
            helper.assertTrue(com.redslovesgames.tideborne.fishing.v2.FishingGearEffects.catchZoneAreaMultiplier(optional) == 1,
