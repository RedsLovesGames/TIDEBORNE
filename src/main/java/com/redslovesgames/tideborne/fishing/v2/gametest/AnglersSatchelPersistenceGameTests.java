...
import com.redslovesgames.tidetraits.satchel.SatchelPreset;
import com.redslovesgames.tidetraits.satchel.SatchelTackleExchange;
import com.redslovesgames.tidetraits.satchel.SatchelTackleHandler;
import com.redslovesgames.tidetraits.satchel.SatchelGearSummary;
...
public final class AnglersSatchelPersistenceGameTests implements FabricGameTest {
    ...
    public void physicalPresetsPersistStacksWithoutChangingCatchStorage(TestContext helper) {
        ...
        AnglersSatchelStorage.setState(satchel, com.redslovesgames.tidetraits.satchel.SatchelState.fromTag(data));
        ...
        AnglersSatchelStorage.setState(restored, com.redslovesgames.tidetraits.satchel.SatchelState.fromTag(malformed));
    ...
    public void tackleStorageAndViewProtocolsRoundTrip(TestContext helper) {
        ...
        for (int protocol : List.of(2, 3)) {
            ...
            var view = new com.redslovesgames.tidetraits.satchel.network.SatchelView(protocol, net.minecraft.util.Hand.OFF_HAND, 42L,
                    true, true, false, false, 0, 0, AnglersSatchelStorage.capacity(satchel), -1, -1, List.of(), List.of(), items,
                    items.stream().map(item -> com.redslovesgames.tidetraits.satchel.network.PersonalRecordView.unavailable()).toList(),
                    java.util.Set.of(), java.util.Set.of(), com.redslovesgames.tidetraits.satchel.network.SatchelNetworkStatus.REFRESHED, "", tackle);
            var buffer = new net.minecraft.network.RegistryByteBuf(io.netty.buffer.Unpooled.buffer(), helper.getWorld().getRegistryManager());
            try {
                var codec = com.redslovesgames.tidetraits.satchel.network.SatchelViewPayload.STREAM_CODEC;
                codec.encode(buffer, new com.redslovesgames.tidetraits.satchel.network.SatchelViewPayload(view));
